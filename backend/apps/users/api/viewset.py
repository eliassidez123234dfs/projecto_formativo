...
class LoginViewSet(viewsets.ViewSet):
    ...
    def login(self, request):
        ...
        # Migra los items del carrito anónimo (sesión) al carrito del usuario
        # para que no se pierdan los productos agregados antes de iniciar sesión
        if request.session.session_key:
            ...
            if session_cart:
                ...
                if not session_cart.user or session_cart.user_id != usuario.id:
                    session_cart.delete()
        # Renueva la sesión para evitar session fixation
        request.session.cycle_key()

        # Genera tokens JWT inyectando token_version para invalidación remota
        ...
        # Cookies httpOnly: el frontend no puede leerlas (protección XSS)
        # access_token: 15 minutos de vida
        # refresh_token: 7 días de vida, ruta restringida a /api/token/refresh/
