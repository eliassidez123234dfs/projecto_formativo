...
class UsuarioManager(BaseUserManager):
    ...
    def _create_user(self, usuario, correo, password=None, contrasena=None, **extra_fields):
        ...
        pwd = password if password is not None else contrasena
        if not pwd:
            raise ValueError('La contraseña es obligatoria')
        usuario_obj = self.model(usuario=usuario, correo=correo, **extra_fields)
        usuario_obj.set_password(pwd)
...
class Usuario(models.Model):
    ...
    def has_usable_password(self):
        ...
        return is_password_usable(self.contrasena)
