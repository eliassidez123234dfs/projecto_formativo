...
def cache_view_action(timeout: int = 300, prefix: str = 'v'):
    ...
    IMPORTANTE: se almacena `response.data` (datos serializables) en lugar del
    objeto `Response` de DRF. Guardar la Response directamente falla con
    `ContentNotRenderedError` porque DRF renderiza el contenido de forma
    perezosa y el objeto no es pickleable antes de eso. Al reconstruir la
    Response desde la caché, `data` sigue siendo accesible para el renderer.
    ...
    def decorator(view_func):
        ...
        def wrapper(self, request, *args, **kwargs):
            ...
            if isinstance(response, HttpResponseBase) and response.status_code == status.HTTP_200_OK:
